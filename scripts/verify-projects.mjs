...
const unfocus=p=>p.evaluate(()=>document.activeElement instanceof HTMLElement&&document.activeElement.blur());
...
async function save(p,name,keyboard=true){const downloading=p.waitForEvent('download');if(keyboard){await unfocus(p);await p.keyboard.press('Control+s');}else{await button(p,'Project menu').click();await button(p,'Save project').click();}const file=await downloading;assert(file.suggestedFilename().endsWith('.snip'));const target=path.join(out,name);await file.saveAs(target);assert.equal(await file.failure(),null);return {buffer:readFileSync(target),target};}
let page;
try{
  ...
  await button(page,'Crop').click();await button(page,'1:1').click();
  await button(page,'Frame').click();await button(page,'Portrait 9:16').click();await page.getByRole('slider',{name:'Inset',exact:true}).fill('6');await page.getByLabel('Custom background color',{exact:true}).fill('#c4d4df');
  ...
  await button(page,'Speed').click();await button(page,'1.5×').click();await page.getByRole('switch',{name:'Keep audio'}).uncheck();
  await button(page,'Export video').click();await page.getByLabel('Export format',{exact:true}).selectOption('webm');await page.getByLabel('Export resolution',{exact:true}).selectOption('360');await page.getByLabel('Export quality',{exact:true}).selectOption('compact');await button(page,'Close export').click();
  ...
  for(const viewport of [{width:320,height:640},{width:390,height:844},{width:844,height:390}]){
    await phone.setViewportSize(viewport);await button(phone,'Project menu').tap();const box=await button(phone,'Save project').boundingBox();assert(box.height>=44);assert.equal(await phone.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);await phone.screenshot({path:path.join(out,`project-menu-${viewport.width}.png`)});await button(phone,'Close project menu').tap();
  }
