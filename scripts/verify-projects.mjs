...
async function fresh(options={}){
  ...
  await page.goto(app);await button(page,'import local project').waitFor();
...
try{
  ...
  await button(other,'Back to editing').click();await button(other,'Project menu').click();await button(other,'Clear saved video').click();await button(other,'Clear video').click();await button(other,'import local project').waitFor();assert.equal(await stored(other),null);
  const openFromEmpty=other.waitForEvent('filechooser');await button(other,'import local project').click();await(await openFromEmpty).setFiles(saved.target);await other.waitForFunction(()=>document.querySelector('video')?.readyState>=2);assert.deepEqual(await stored(other),expected);log('A downloaded project restores work after browser autosave is cleared');

  const phone=await fresh({viewport:{width:390,height:844},isMobile:true,hasTouch:true,deviceScaleFactor:1});await phone.context().setOffline(true);const mobileChooser=phone.waitForEvent('filechooser');await button(phone,'import local project').tap();await(await mobileChooser).setFiles(saved.target);await phone.waitForFunction(()=>document.querySelector('video')?.readyState>=2);assert.deepEqual(await stored(phone),expected);
