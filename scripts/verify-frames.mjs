...
try{
  ...
  for(const rate of ['60','variable']){
    ...
    execFileSync(ffmpeg,['-v','error','-f','lavfi','-i','testsrc2=size=160x90:rate=60:duration=2',...(rate==='variable'?['-vf',"select='if(lt(n,60),not(mod(n,2)),not(mod(n,3)))'",'-fps_mode','vfr']:[]),'-c:v','libx264','-bf',rate==='variable'?'0':'3','-y',fixture]);
