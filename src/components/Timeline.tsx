...
export default function Timeline({zoom,onZoom,onUndo,onRedo,canUndo,canRedo,source,edits,time,selected,frames,onSelect,onSeek,onClips,onCheckpoint,onSplit,onDelete,canSplit}:Props){
    ...
    <div ref={scroll} className="timeline-scroll"><div className="timeline-inner" style={{width:`${zoom*100}%`}}><div className="ruler" onPointerDown={startScrub} onPointerMove={e=>{if(scrubbing.current===e.pointerId)seek(e);}} onPointerUp={end} onPointerCancel={end}>{ticks.map(t=><span key={t} style={{left:`${t/Math.max(duration,.001)*100}%`}}>{formatTime(t,tickStep<1)}</span>)}</div>
      <div ref={track} className="clip-track">
        {edits.clips.map((clip,index)=>{
          ...
          return <div key={clip.id} className={`timeline-clip ${selected===clip.id?'selected':''}`} style={{left:`${left}%`,width:`${width}%`}} role="button" tabIndex={0} aria-label={`Clip ${index+1}, ${formatTime((clip.end-clip.start)/edits.speed)}`} aria-pressed={selected===clip.id} onClick={()=>onSelect(clip.id)} onPointerDown={e=>{if(e.pointerType==='touch'){if(!touch.current)touch.current={pointer:e.pointerId,x:e.clientX,y:e.clientY};return;}onSelect(clip.id);startScrub(e);}} onPointerMove={e=>{if(scrubbing.current===e.pointerId)seek(e);}} onPointerUp={e=>{const t=touch.current;if(t?.pointer===e.pointerId){if(Math.hypot(e.clientX-t.x,e.clientY-t.y)<10){onSelect(clip.id);seek(e);}touch.current=null;}end(e);}} onPointerCancel={e=>{touch.current=null;end(e);}} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();e.stopPropagation();onSelect(clip.id);onSeek(left/100*duration);}}}>
