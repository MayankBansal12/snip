import { boundary, retainedFrames, trimBoundary, stepBoundary, preciseTime } from '../frame-timing';
...
type Props = { frameIndex:FrameIndex|null; onFrameStep:(direction:number)=>void; chatActive: boolean; chatEditor: ReactNode; modeSwitch: ReactNode; videoRef: RefObject<HTMLVideoElement | null>; zoom:number; onZoom:(zoom:number)=>void; source: Source; edits: Edits; time: number; selected: string; frames: { url: string; time: number }[]; onSelect: (id: string) => void; onSeek: (time: number) => void; onClips: (clips: Clip[]) => void; onCheckpoint: () => void; onSplit: () => void; onDelete: () => void; onResetTrim: () => void; onUndo: () => void; onRedo: () => void; canUndo: boolean; canRedo: boolean; canSplit: boolean; actionsOpen: boolean; onActionsOpen: (open: boolean) => void; onOpenClip: (id: string) => void; onChangeClip: (patch: Partial<Clip>, record?: boolean) => void; onMerge: (index: number) => void };
export default function Timeline({frameIndex,onFrameStep,chatActive,chatEditor,modeSwitch,videoRef,zoom,onZoom,source,edits,time,selected,frames,onSelect,onSeek,onClips,onCheckpoint,onSplit,onDelete,canSplit,actionsOpen,onActionsOpen,onOpenClip,onChangeClip,onMerge,onResetTrim,onUndo,onRedo,canUndo,canRedo}:Props){
  ...
  const trimTo=(clips:Clip[],index:number,edge:'start'|'end',value:number)=>{
    ...
    clip[edge]=frameIndex?trimBoundary(frameIndex,clip,edge,value,trimBounds(next,clip,source.duration)):edge==='start'?clamp(value,trimBounds(next,clip,source.duration).start,clip.end-minClip):clamp(value,clip.start+minClip,trimBounds(next,clip,source.duration).end);
    onClips(next);
    ...
    const previewTime=frameIndex?boundary(frameIndex,edge==='start'?retainedFrames(frameIndex,clip).first:retainedFrames(frameIndex,clip).after-1):edge==='start'?clip.start:Math.max(clip.start,clip.end-.001);
