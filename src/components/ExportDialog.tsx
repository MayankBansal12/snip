...
export default function ExportDialog(p: Props) {
  ...
  return <Dialog open={p.open} onOpenChange={open => { if (!open && !p.busy) p.onClose(); }}>
    <DialogPopup className="export-dialog" showCloseButton={!p.busy} closeProps={{ 'aria-label': 'Close export' }}>
      ...
      <DialogPanel>
        {p.download ? <Empty className="py-6 md:py-6"><EmptyHeader><EmptyMedia variant="icon"><Check className="text-success-foreground" /></EmptyMedia><EmptyTitle>Export complete</EmptyTitle><EmptyDescription>{output.width} × {output.height} · {formatTime(sequenceDuration(p.edits))} · {filesize(p.download.size)}</EmptyDescription></EmptyHeader></Empty> : p.busy ? <div className="py-8"><Progress value={Math.round(p.progress*100)}><div className="flex justify-between gap-4"><ProgressLabel>{p.stage || 'Preparing export…'}</ProgressLabel><ProgressValue /></div><ProgressTrack><ProgressIndicator /></ProgressTrack></Progress></div> : <div className="flex flex-col gap-5">
