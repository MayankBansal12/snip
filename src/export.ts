...
import { zoomTransitionFilter } from './zoom';
...
export async function exportVideo(source: Source, edits: Edits, progress: (fraction: number, stage: string) => void): Promise<Blob> {
  ...
  try {
    ...
    edits.clips.forEach((clip,i)=>{
      ...
      const transition=zoomTransitionFilter(source,edits,i,frameRate||30);
      const picture=[`trim=start=${clip.start}:end=${clip.end}`,'setpts=PTS-STARTPTS'];
      // Perspective expressions use frame counts. Normalize only transitioning
      // clips to the source's nominal frame rate before retiming them.
      if(transition)picture.push(`fps=${frameRate||30}:round=near`);
      // fps uses a coarse 1/fps time base; restore precision before custom retiming.
      picture.push('settb=AVTB', `setpts=PTS/${speed}`);
      if(transition)picture.push(transition);
