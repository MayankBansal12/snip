...
Chat contains a prompt field with send, undo, and redo. Feedback appears only while processing or after an edit. For manual zoom adjustments, switch to the timeline and open **zoom** to drag the focus box. “Zoom needs to be in top left side” keeps an existing magnification; without a zoom, it starts at 1.5×. “Trim 5 seconds” removes the first five seconds; “trim to 5 seconds” keeps the first five. The result states the applied edit.
...
With a running server, `node --import tsx scripts/verify-chat-prompts.ts` checks common prompts against real Jev responses, including repeated zoom/speed changes, multi-split plans, per-clip trims, merges, new-part references, relative timing, and edited timelines. Use `PROMPT_SUITE=ordered` to run only the compound-plan cases, or `PROMPT_SUITE=numeric` for number-led settings such as “1x zoom and 1x speed for clip 1”. The full suite pauses between request windows to respect the server rate limit. Set `SNIP_URL` to override `http://127.0.0.1:52947`.
